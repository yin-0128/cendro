...
COMMITTED = ["seed_pairs.jsonl", "gold_pairs.jsonl", "train_pairs.jsonl"]
...
def test_train_pairs_is_seed_plus_gold():
    n = {name: len(load_jsonl(DATA / name)) for name in COMMITTED}
    assert n["train_pairs.jsonl"] == n["seed_pairs.jsonl"] + n["gold_pairs.jsonl"]
